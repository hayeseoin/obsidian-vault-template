...
const TEMPLATE_ARG = args.find(arg => arg.startsWith('--template='));
const FORCE_MODE = args.includes('--force');

if (!TEMPLATE_ARG) {
  console.error('❌ Missing required argument: --template=template.md');
  process.exit(1);
}

const TEMPLATE_NAME = TEMPLATE_ARG.split('=')[1];
const TEMPLATE_FILE = join('assets/templates/dataview', TEMPLATE_NAME);
